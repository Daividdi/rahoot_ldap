...
const Username = () => {
  ...
  const loadHistory = () => {
    ...
    ;(socket as any).emit("player:getHistory", { realName: storedName })
    const handler = (data: any) => {
      setHistoryData(data)
      setHistoryLoading(false)
      ;(socket as any).off("player:history", handler)
    }
  ...
  useEvent("game:successJoin", (gameId) => {
    ...
    setStatus(STATUS.WAIT, { text: "Waiting for the game to start..." })
    ...
    <div className="card-3d z-10 flex w-full max-w-md flex-col gap-4 rounded-2xl bg-white p-6">
      ...
      <div className="flex gap-1 rounded-xl bg-gray-100 p-1">
        <button
          ...
          className={`flex-1 rounded-lg py-2 text-xs font-bold transition-colors ${mainTab === "avatar" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          Avatar
        </button>
        <button
          onClick={() => { setMainTab("history"); if (!historyData) loadHistory() }}
          className={`flex-1 rounded-lg py-2 text-xs font-bold transition-colors ${mainTab === "history" ? "bg-white text-primary shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
        >
          History
        </button>
      </div>
      ...
      {mainTab === "history" && (
        <div className="flex flex-col gap-3" style={{ maxHeight: "65vh", overflowY: "auto" }}>
            ...
            <>
              ...
              <div>
                ...
                {historyData.leaderboard.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-1">
                    {historyData.leaderboard.map((p: any, i: number) => (
                      <div key={i} className={`flex items-center gap-2 rounded-xl px-3 py-2 ${(p.realName || "").toLowerCase() === storedName.toLowerCase() ? "bg-primary/10 border border-primary/20" : "bg-gray-50"}`}>
                        <span className={`w-5 text-center text-xs font-bold shrink-0 ${i === 0 ? "text-amber-500" : i === 1 ? "text-gray-400" : i === 2 ? "text-amber-700" : "text-gray-400"}`}>{i + 1}</span>
                        <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-primary/15 text-[10px] font-bold text-primary">
              ...
              {historyData.history.length > 0 && (
                <div>
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Recent Games</p>
                  <div className="flex flex-col gap-1.5">
                    {historyData.history.map((s: any, i: number) => (
                      <div key={i} className="rounded-xl bg-gray-50 border border-gray-100 px-3 py-2">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs font-semibold text-gray-700 flex-1 truncate">{s.quizTitle}</p>
                          <span className="text-xs font-bold text-primary shrink-0">{s.player.points} pts</span>
                        </div>
              ...
              {historyData.history.length === 0 && (
                <p className="text-xs text-gray-400 text-center py-3">You haven't played any games yet.</p>
              )}
            </>
          )}
          ...
          <button
            onClick={loadHistory}
            className="mt-1 w-full rounded-xl border border-gray-200 bg-gray-50 py-2 text-xs font-semibold text-gray-500 hover:bg-gray-100 transition-colors"
          >
            Refresh
          </button>
        </div>
